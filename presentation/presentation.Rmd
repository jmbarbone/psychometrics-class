...
Adaptive and Aggressive Assertiveness Scale (AAA-S) [@thompson2011adaptive]

Basic Empathy Scale in Adults (BES-A) [@carre2013basic]

Leadership Self-Report Scale (LSRS) [@dussault2013leadership]

Narcissism Personality Inventory (NPI-16) [@ames2006npi]
...
csv %>%
  ...
  pluck("Alpha")
csv %>%
  filter(scale == "CON", item %in% keep_items) %>%
  spread(item, response) %>% 
  select(-c(id:domain)) %>% 
  cronbach %>% 
  pluck("Alpha")
...
processed %>% 
  ...
  unnest(tidy) %>% select(-data, -cor_test, -method, -estimate) %>% 
  kable_scroll("250px", digits = 3)
```
